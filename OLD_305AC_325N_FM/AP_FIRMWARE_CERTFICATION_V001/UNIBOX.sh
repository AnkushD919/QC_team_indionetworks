...
Get_APList1 'bash FMVerificationprocess.sh'

Report_SummaryLog "MAC       FM Verison check -  PING Results -  Braodcast Check -    "

echo "Starting Certification in Parallel Collected  AP IP List"
...
CR_File=$(sshpass -p "$APPWD" ssh -o StrictHostKeyChecking=no root@"$AP_IP"<<EOF 2> "$AP_IP"errors.txt
cd /tmp
sleep 5
ls
exit
...
ssh-keygen -f "/home/indio/.ssh/known_hosts" -R "$AP_IP"

sleep 12

sshpass -p 'yes' ssh -o StrictHostKeyChecking=no root@"$AP_IP"
...
CR_File=$(sshpass -p "$APPWD" ssh -o StrictHostKeyChecking=no root@"$AP_IP"<<EOF 
cd /tmp
sleep 10
ls
exit
...
#Get_IP "172.31.254.115" "70:6d:ec:13:00:f5"

#line="? (10.10.10.6) at 70:6d:ec:13:00:f5 [ether] on Madhav_test"

#Result22=$(cut -d "(" -f2 <<< "$line")
  
#echo "$Result22"


#SUBSTRING=$(echo $Result22| cut -d")" -f1)
#echo $SUBSTRING
...
CR_File=$(sshpass -p "$APPWD" ssh -p 2233 "$APUID"@"$AP_IP"<<EOF 2> "$AP_IP"errors.txt
cd /tmp
sleep 2
ls
exit
...
ssh-keygen -f "/home/indio/.ssh/known_hosts" -R "[$AP_IP]:2233"


sleep 2

#sshpass -p 'yes' ssh -p 2233 "$APUID"@"$AP_IP"
