...
Report_SummaryLog " "$mc"     "$FM-"            "$CER-"                      "$CONF-"              "$ping-"         "$Broadcast"          "$Port""
...
# manually password
#sudo nmcli dev wifi connect "$SSID"$id

# Automatic password
#echo 'wifi123#' | sudo -S nmcli dev wifi connect Maverick-1300F4


#Getting SSID
<<com
checkconfile=$(sshpass -p "$APPWD" ssh -o StrictHostKeyChecking=no root@"$AP_IP"<< EOF_run_commands 
...
printf "Result is=%s" "$checkconfile"

echo "$checkconfile" | tee "$AP_IP""SSID".txt

Result=$(grep wlan0  "$AP_IP""SSID".txt)

echo "$Result"

printf "ssid is =%s" "$Result"
