...
sshpass -p "$APPWD" ssh -p 2233 "$APUID"@"$AP_IP"<< EOF_run_commands

sleep 5
		
ifconfig wlan-hs24 | tee /tmp/CResult001.txt
...
sshpass -p "$APPWD" scp -P 2233 "$APUID"@"$AP_IP":/tmp/CResult001.txt .
