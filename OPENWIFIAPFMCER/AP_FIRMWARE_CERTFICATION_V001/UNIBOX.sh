...
Get_APList 'bash FMCERVerificationprocess.sh'
...
CR_File=$(sshpass -p 'openwifi' ssh -o StrictHostKeyChecking=no root@"$AP_IP"<<EOF 2> "$AP_IP"errors.txt
cd /tmp
sleep 5
ls
exit
...
CR_File=$(sshpass -p 'openwifi' ssh -o StrictHostKeyChecking=no root@"$AP_IP"<<EOF 
cd /tmp
sleep 10
ls
exit
